...
if __name__ == '__main__':
    ...
    parser.add_option("--scfile",dest="scfilename", default='L1701271500537C62B87191_SC00.list')
    ...
    parser.add_option("--model",dest="modeloutbase",default='BLLac_3FGL_modelPL_out')
    ...
    bllac3fgl = analyze3FGL(name=options.name, id_string=options.source_name, ra=options.ra, dec=options.dec,
                 ...
                 model=options.model, modeloutbase=options.modeloutbase, rad=options.rad, srcrad=options.srcrad)
    #t_start = hdulist[1].header['TSTART'] # TSTART time from spacecraft file
    #t_stop = hdulist[1].header['TSTOP'] # TSTOP time from spacecraft file
    ...
    bllac3fgl.run(t_start=options.t_start, t_stop=options.t_stop)
