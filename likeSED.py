...
def getSED(sed, plot=True, plot_sed=False, save_spec=None, save_sed=None, eminGeV=0.1, emaxGeV=300):
    ...
    XTitle = 'Energy (GeV)'
    YTitle = [r'dN/dE (cm$^{-2}$ s$^{-1}$ GeV$^{-1}$ )', r'E$^{2}$dN/dE (erg cm$^{-2}$ s$^{-1}$ )',
              r'$\nuF_{\nu}$ (erg cm$^{-2}$ s$^{-1}$ )']
    # let's make the graphs for the model, both the counts spectrum and the E^2dN/dE spectrum
    modBins = log_array(51, sed.FitBounds[0] / 1000., sed.FitBounds[1] / 1000.)
    modelEcent = [0.5 * (e1 + e2) for e1, e2 in zip(modBins[:-1], modBins[1:])]
    ...
    if len(sed.data[3]) == 0:
        ...
        for m, M, f, e, g, c in zip(minEs, maxEs, fluxPts, fluxErrs, sed.data[3], ecent):
            if g != 1 and f != 0:
                newfluxPts += [f * (-g + 1) * (c * 1000.) ** (-g) / (M ** (-g + 1) - m ** (
                -g + 1))]  # works out to be prefactor assuming Scale parameter (E0) of c (center en
                newfluxErrs += [
                    e * (-g + 1) * (c * 1000.) ** (-g) / (M ** (-g + 1) - m ** (-g + 1))]  # similar for the errors
            else:
                if f != 0:
                    newfluxPts += [f / (c * log(M / m))]
                    newfluxErrs += [e / (c * log(M / m))]
    ...
    if plot:
        ...
        ax.errorbar(ecent, GeVfluxPts, xerr=[Eminus, Eplus], yerr=GeVfluxErrs, color='r', ecolor='r', capthick=0,
                    fmt='.')  # , label="GeVfluxPts")
        ax.plot(EGeV, FGeV, 'r--')
    ...
    if plot_sed:
        ...
        ax.errorbar(ecent, EsqFluxes, xerr=[Eminus, Eplus], yerr=EsqFErrors, color='b', ecolor='b', capthick=0, fmt='.',
                    label="LAT")

        ax.plot(EGeV, (EGeV) ** 2 * FGeV * gev2erg, 'b--')
        ax.fill_between(EGeV, (EGeV) ** 2 * (FGeV - FGeVerr) * gev2erg, (EGeV) ** 2 * (FGeV + FGeVerr) * gev2erg,
                        alpha=0.3, color='b')
